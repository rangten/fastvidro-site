...
export const Route = createFileRoute("/espelhos/$modelo")({
  loader: ({ params }): LoaderData => {
    const model = getModel("espelhos", params.modelo);
  ...
  head: ({ loaderData, params }) => {
    if (!loaderData) return { meta: [{ title: "Espelhos — Fast Vidro" }] };
    if (loaderData.kind === "model") {
      const { model } = loaderData.data;
      return {
        meta: [
          { title: `Espelho ${model.name} — Fast Vidro` },
          { name: "description", content: model.description },
          { property: "og:title", content: `Espelho ${model.name} — Fast Vidro` },
  ...
  component: EspelhoDispatch,
});

function EspelhoDispatch() {
  ...
  return <EspelhoBairroPage nome={loaderData.nome} zona={loaderData.zona} />;
}

function EspelhoBairroPage({ nome, zona }: { nome: string; zona: ZonaInfo }) {
